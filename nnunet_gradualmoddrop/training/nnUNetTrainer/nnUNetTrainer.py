...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int,prob:float, mod:str,gradual:int,dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.mod = mod
        self.mod = [i for i in self.mod.split(',')] 
    ...
    def get_plain_dataloaders(self, initial_patch_size: Tuple[int, ...], dim: int):
        ...
        if dim == 2:
            ...
            dl_tr = nnUNetDataLoader3D(dataset_tr, self.batch_size,
                                       ...
                                       self.label_manager,
                                       oversample_foreground_percent=self.oversample_foreground_percent,
                                       sampling_probabilities=None, pad_sides=None, epoch=self.current_epoch,dataset='train')
            dl_val = nnUNetDataLoader3D(dataset_val, self.batch_size,
                                        self.configuration_manager.patch_size,
                                        self.configuration_manager.patch_size,
                                        self.label_manager,
                                        oversample_foreground_percent=self.oversample_foreground_percent,
                                        sampling_probabilities=None, pad_sides=None,epoch=self.current_epoch)
        return dl_tr, dl_val
    ...
    def train_step(self, batch: dict,dataset:str='val'):
        data = batch['data']
        target = batch['target']
        ...
        if self.gradual>0:
            ...
            if self.current_epoch<250:
                mask=0.25
                #porc = 8
            elif self.current_epoch<500:
                mask=0.5
                #porc = 6
            elif self.current_epoch<750:
                mask =0.75
                #porc = 3
            else:
                mask=1.0
        ...
        if dataset=='train':
                ...
                for k in range(data.shape[0]):
                    ...
                    if kk<prob:
                    #data[2]=np.zeros((data.shape[1:]))
                       for i in range(len(self.mod)):
                            
                        
                            mask = torch.zeros((data.shape[2:]))+(1-mask)+ np.random.normal(0,0.1, size=1)
                            data[k,int(self.mod[i])]=data[k,int(self.mod[i])]*mask
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                train_outputs.append(self.train_step(next(self.dataloader_train),dataset='train'))
