...
class nnUNetPredictor(object):
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                prediction = self.predict_logits_from_preprocessed_data(data, file_name).cpu()
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor,file_name) -> torch.Tensor:
        """
                ...
                try:
                    for params in self.list_of_parameters:
                        ...
                        if prediction is None:
                            prediction = self.predict_sliding_window_return_logits(data,file_name)
                        else:
                            prediction += self.predict_sliding_window_return_logits(data,file_name)
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor,file_name) -> torch.Tensor:
        ...
        if mirror_axes is not None:
            ...
            if 0 in mirror_axes:
                prediction_= self.network(torch.flip(x, (2,)))
                prediction +=torch.flip(prediction_, (2,))
            if 1 in mirror_axes:
                prediction_=self.network(torch.flip(x, (3,)))
                prediction += torch.flip(prediction_, (3,))
            if 2 in mirror_axes:
                prediction_=self.network(torch.flip(x, (4,)))
                prediction += torch.flip(prediction_, (4,))
            if 0 in mirror_axes and 1 in mirror_axes:
                prediction_=self.network(torch.flip(x, (2, 3)))
                prediction += torch.flip(prediction_, (2, 3))

            if 0 in mirror_axes and 2 in mirror_axes:
                prediction_=self.network(torch.flip(x, (2, 4)))
                prediction += torch.flip(prediction_, (2, 4))
            if 1 in mirror_axes and 2 in mirror_axes:
                prediction_=self.network(torch.flip(x, (3, 4)))
                prediction += torch.flip(prediction_, (3, 4))
            if 0 in mirror_axes and 1 in mirror_axes and 2 in mirror_axes:
                prediction_=self.network(torch.flip(x, (2, 3, 4)))
                prediction += torch.flip(prediction_, (2, 3, 4))
            prediction /= num_predictons
        return prediction

    def predict_sliding_window_return_logits(self, input_image: torch.Tensor,file_name) \
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                for sl in tqdm(slicers, disable=not self.allow_tqdm):
                    ...
                    prediction = self._internal_maybe_mirror_and_predict(workon,file_name)[0].to(results_device)
