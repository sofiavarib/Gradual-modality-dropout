...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
                          configuration: str,
                          fold: int,prob: float,mod:str,gradual:int,
    ...
    nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,prob=prob,mod=mod,gradual=gradual,
...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p,mod, use_compressed, disable_checkpointing, c, val,
            pretrained_weights, npz, val_with_best, world_size):
    setup_ddp(rank, world_size)
    torch.cuda.set_device(torch.device('cuda', dist.get_rank()))

    nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, tr, p,mod,
                                           use_compressed)
...
def run_training(dataset_name_or_id: Union[str, int],
                 ...
                 prob: float=0,mod:str='',gradual:int=0,
    ...
    if num_gpus > 1:
        ...
        print(dataset_name_or_id, configuration, fold, prob,mod, gradual,trainer_class_name)
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, prob,mod,gradual,trainer_class_name,
...
def run_training_entry():
    ...
    parser.add_argument('-prob', type=float,default=0.0,required=False, 
                        help="Probability gradual modality dropout")
    parser.add_argument('-mod', type=str,default=0,required=False, 
                        help="Modality gradual modality dropout")
    parser.add_argument('-gradual', type=int,default=0,required=False, 
                        help="Modality gradual modality dropout")
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.prob,args.mod, args.gradual,args.tr, args.p,args.pretrained_weights,
